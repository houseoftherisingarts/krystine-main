...
const WaitlistModal: React.FC<WaitlistModalProps> = ({ target, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#0B1A36]/60 backdrop-blur-md" onClick={close}>
      <div
        className="relative w-full max-w-md bg-white dark:bg-[#0B1A36] rounded-[30px] shadow-2xl border border-[#D4AF37]/20 p-8 md:p-10"
        onClick={e => e.stopPropagation()}
      >
        <button
          onClick={close}
          className="absolute top-5 right-5 w-9 h-9 rounded-full flex items-center justify-center text-[#0B1A36]/40 dark:text-white/40 hover:text-[#0B1A36] dark:hover:text-white"
        ...
        <h2 className="font-serif text-3xl text-[#0B1A36] dark:text-white mb-2">
          {lang === 'FR' ? "Liste d'attente" : 'Join the waitlist'}
        </h2>
        <p className="text-sm text-[#0B1A36]/60 dark:text-white/60 mb-6 font-serif italic">
          {lang === 'FR'
        ...
        {done ? (
          <div className="text-center py-6">
            ...
            <p className="font-serif italic text-[#0B1A36] dark:text-white">
              {lang === 'FR'
            ...
            <button
              onClick={close}
              className="mt-6 inline-flex items-center gap-2 bg-[#0B1A36] dark:bg-[#D4AF37] text-white dark:text-[#0B1A36] px-8 py-3 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-[#D4AF37] hover:text-[#0B1A36] transition-colors"
            >
          ...
          <form onSubmit={submit} className="space-y-3">
            <input
              ...
              className="w-full px-5 py-3 rounded-full border border-[#0B1A36]/15 dark:border-white/15 bg-white dark:bg-white/5 text-sm text-[#0B1A36] dark:text-white placeholder:text-[#0B1A36]/40 dark:placeholder:text-white/40 focus:outline-none focus:border-[#D4AF37]"
            />
            <input
              type="email"
              required
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full bg-[#0B1A36] dark:bg-[#D4AF37] text-white dark:text-[#0B1A36] px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-[#D4AF37] hover:text-[#0B1A36] transition-colors disabled:opacity-60"
            >
              {busy
            ...
            <p className="text-[10px] text-center text-[#0B1A36]/50 dark:text-white/50 pt-1">
              {lang === 'FR'
