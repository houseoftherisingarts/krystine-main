...
// choix, d'un clic). Alex, 6 septembre 2026.
const FondEcran: React.FC<{ ouvert: boolean; onFermer: () => void; image: string; nom: string; lang: string }> = ({ ouvert, onFermer, image, nom, lang }) => {
  const fr = lang === 'FR';
  ...
  return (
    <AnimatePresence>
      {ouvert && (
        <Portail>
        <motion.div className="fixed inset-0 z-[130] flex items-center justify-center bg-[#151d19]/70 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onFermer}>
          <motion.div
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <a href={image} download={fichier} className="inline-flex items-center gap-2 rounded-full bg-[#293027] px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest text-[#EEE7DB] hover:bg-[#3a453a] dark:bg-[#BA7B39] dark:text-[#293027] dark:hover:bg-[#d9a05b]">
                <i className="fa-solid fa-download" /> {fr ? 'Garder l’image' : 'Keep the image'}
              </a>
