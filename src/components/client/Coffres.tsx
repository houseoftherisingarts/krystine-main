...
import { COFFRES, ORDRE_COFFRES, OUVERTURES_PAR_JOUR, NISKAS_MUSIQUE_DEJA, PRIX_CLE, chanceLisible, texteCosmetique, type TypeCoffre } from '../../lib/coffresConfig';
                ...
                {contenuOuvert === type && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35, ease }} className="overflow-hidden text-sm">
                    ...
                    <ul className="mt-1 space-y-1.5 text-[#293027] dark:text-white">
                      <li className="flex items-start gap-2"><i className="fa-solid fa-palette mt-1 text-[10px] text-[#8B4A2F] dark:text-[#d9a05b]" /><span>{texteCosmetique(type, enTravail, fr ? 'FR' : 'EN')}</span></li>
        ...
        {ouverture && (
          ...
          <motion.div className="fixed inset-0 z-[130] flex items-center justify-center overflow-y-auto overscroll-contain bg-[#151d19]/70 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => ouverture.lots && !(grand && !grandOk) && setOuverture(null)}>
            ...
            >
              ...
              {/* La figurine transparente (bronze/argent/or.webp, vrai canal
                  alpha), jamais la vidéo d'ouverture : celle-ci a un fond noir
                  cuit dans l'image, qui se voyait derrière le coffre sur la
                  carte ivoire (Alex, 7 septembre 2026). */}
