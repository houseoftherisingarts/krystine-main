...
const Coffres: React.FC<{ solde: number; onChange?: () => void }> = ({ solde, onChange }) => {
  ...
  return (
    <div className="mt-8" id="boutique-coffres">
      ...
      <AnimatePresence>
        {ouverture && (
          <motion.div className="fixed inset-0 z-[130] flex items-center justify-center bg-[#151d19]/70 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => ouverture.lot && ouverture.lot.genre !== 'grand' && setOuverture(null)}>
            <motion.div
