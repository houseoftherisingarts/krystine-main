...
import { acheterAvecNiskas, acheterNiskas, acheterSaisonEnArgent, subscribeToMemberPoints, subscribeToSkinsSettings, suivreBoutique, type PointsBalance } from '../../firebase/points';
import {
  CATEGORIES_BOUTIQUE, skinParCle, banniereParCle, SKINS, skinEnTravail, type SkinsSettings,
          ...
          return carte(a.id, (
            <>
              ...
              <div className="flex flex-1 flex-col p-4">
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#8B4A2F]"><i className={`fa-solid ${a.icone} mr-1`} /> {a.categorie === 'skin' && skinParCle(a.id.slice(5))?.cout === null ? (skinParCle(a.id.slice(5))?.rarete === 'exclusif' ? (fr ? 'Exclusif · Badge Bleu' : 'Exclusive · Blue Badge') : skinParCle(a.id.slice(5))?.rarete === 'legendaire' ? (fr ? 'Légendaire · coffre d’or' : 'Legendary · gold chest') : (fr ? `Rare · coffre ${skinParCle(a.id.slice(5))?.coffre === 'argent' ? 'd’argent' : 'd’or'}` : `Rare · ${skinParCle(a.id.slice(5))?.coffre} chest`)) : a.cout === 0 ? (fr ? 'Offerte' : 'Included') : niskas(a.cout, lang)}</p>
