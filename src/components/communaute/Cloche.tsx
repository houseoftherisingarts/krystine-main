...
const Cloche: React.FC<{ uid: string }> = ({ uid }) => {
  ...
  return (
    <div ref={boite} className="relative flex items-center gap-2">
      <button
        type="button" onClick={() => { setOuverte((v) => !v); setMessagesOuverts(false); }} className={`${bouton} inline-flex`} style={{ ...style, color: '#57524c' }}
      ...
      <AnimatePresence>
        {messagesOuverts && (
          <motion.div
            ...
            className="absolute right-0 top-[calc(100%+10px)] z-30 w-[min(22rem,calc(100vw-2rem))] max-sm:fixed max-sm:inset-x-4 max-sm:top-[72px] max-sm:w-auto rounded-[20px] overflow-hidden bg-[#EEE7DB] dark:bg-[#293027] border border-[#BA7B39]/25 shadow-[0_18px_50px_-20px_rgba(41,48,39,0.45)]"
            role="menu"
          >
            ...
            <Link
              to={versMessages} onClick={() => setMessagesOuverts(false)}
      ...
      <AnimatePresence>
        {ouverte && (
          <motion.div
            ...
            transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
            className="absolute right-0 top-[calc(100%+10px)] z-30 w-[min(22rem,calc(100vw-2rem))] max-sm:fixed max-sm:inset-x-4 max-sm:top-[72px] max-sm:w-auto rounded-[20px] overflow-hidden bg-[#EEE7DB] dark:bg-[#293027] border border-[#BA7B39]/25 shadow-[0_20px_50px_-20px_rgba(41,48,39,0.4)]"
            role="menu"
          >
            <p className="px-4 pt-3.5 pb-2 text-[10px] uppercase tracking-[0.18em] text-[#38403a]/50 dark:text-white/45 border-b border-[#38403a]/10 dark:border-white/10">
              Ce qui vous attend
