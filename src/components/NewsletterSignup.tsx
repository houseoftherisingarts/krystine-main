...
const NewsletterSignup: React.FC<Props> = ({
  ...
  return (
    <div className={className}>
      ...
      {accountUpsell && (
        <div className={`mt-6 border-t pt-6 text-center ${isDark ? 'border-white/10' : 'border-[#2a2015]/10'}`}>
          <p className={`text-sm ${fineprintClass}`}>
            {lang === 'FR'
              ? 'Vous voulez plus qu\'une infolettre ? Contenus exclusifs et promotions spéciales :'
              : 'Want more than a newsletter? Exclusive content and special offers:'}
          </p>
          <Link
            to="/compte"
