...
const DECOR = /^(modules?|lessons?|mark as complete|great job! keep going!|next lesson|play now|cancel|will begin in \d+ seconds?|\/|\d+|\d+:\d{2}(:\d{2})?)$/i;
...
export function nettoyerKajabi(texte: string, titreLecon?: string, titreFormation?: string): string {
  if (!texte) return texte;
  const lignes = texte.split(/\r?\n/);
  const cibles = [titreLecon, titreFormation].filter(Boolean).map(t => plat(t as string));
