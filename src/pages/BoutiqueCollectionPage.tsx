  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#050C1A] pt-20">
      ...
      <div className="relative w-full h-[55vh] md:h-[60vh] overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#050C1A] via-[#050C1A]/50 to-[#050C1A]/20" />
        <div className="relative z-10 text-center text-white px-6 max-w-3xl">
          <Link to="/boutique" className="inline-flex items-center gap-2 text-[#D4AF37] uppercase tracking-[0.3em] text-[10px] font-bold mb-6 hover:text-white transition-colors">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 pb-24">
        ...
        <section className="py-16 md:py-20 text-center">
          <p className="font-serif italic text-xl md:text-2xl leading-relaxed text-[#0B1A36]/80 dark:text-white/80 max-w-3xl mx-auto">
            {story}
          </p>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-24">
            ...
            <Link
              to="/boutique"
              className="inline-flex items-center gap-2 text-[#D4AF37] uppercase tracking-[0.3em] text-[11px] font-bold hover:text-[#0B1A36] dark:hover:text-white transition-colors"
            >
              ...
              return (
                <div key={product.id} className="group flex flex-col relative">
                  <button
                    ...
                    className="text-left block relative aspect-[3/4] rounded-[24px] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 mb-5 bg-[#F5F5F0] dark:bg-[#0B1A36] cursor-pointer"
                  >
                    <div
                  ...
                  <div className="text-center px-2 cursor-pointer" onClick={() => openProduct(product)}>
                    {product.productType && (
                      <span className="text-[10px] text-[#0B1A36]/50 dark:text-white/50 uppercase tracking-[0.25em] font-bold">{product.productType}</span>
                    )}
                  ...
                  {!soldOut ? (
                    <button
                      ...
                      className="mt-4 w-full bg-[#0B1A36] dark:bg-[#D4AF37] text-white dark:text-[#0B1A36] py-3 rounded-full text-[11px] font-bold uppercase tracking-widest hover:bg-[#D4AF37] hover:text-[#0B1A36] transition-colors shadow-md"
                    >
                    ...
                    <button
                      type="button"
                      disabled
                      className="mt-4 w-full bg-transparent border border-[#0B1A36]/20 dark:border-white/20 text-[#0B1A36]/50 dark:text-white/50 py-3 rounded-full text-[11px] font-bold uppercase tracking-widest cursor-not-allowed"
                    >
                      {lang === 'FR' ? 'Épuisé' : 'Sold out'}
                    </button>
                  )}
        ...
        <div className="mt-24 pt-16 border-t border-[#0B1A36]/10 dark:border-white/10 text-center">
          <span className="text-[#D4AF37] uppercase tracking-[0.3em] text-[10px] font-bold block mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {COLLECTIONS.filter(c => c.slug !== manifest.slug).map(c => (
              <Link
                key={c.slug}
                to={`/boutique/${c.slug}`}
                className="text-xs uppercase tracking-[0.25em] font-bold px-5 py-2.5 rounded-full border border-[#0B1A36]/15 dark:border-white/15 text-[#0B1A36]/70 dark:text-white/70 hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors"
              >
