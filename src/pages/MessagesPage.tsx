...
const MessagesPage: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-[#f6f3ee] dark:bg-[#16100a] pt-32 pb-24 px-6">
        <div className="max-w-md mx-auto text-center">
          <h1 className="font-serif text-3xl text-[#2a2015] dark:text-white mb-4">Messages</h1>
          <p className="text-[#3a3126]/60 dark:text-white/60 mb-8">Connectez-vous pour retrouver vos conversations.</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-[#f6f3ee] dark:bg-[#16100a] pt-24 pb-10">
      <div className="max-w-screen-xl mx-auto px-3 md:px-6">
        <div className="flex items-center gap-3 mb-4">
          <Link to="/espace" className="inline-flex items-center gap-1.5 text-xs uppercase tracking-widest text-[#3a3126]/60 dark:text-white/60 hover:text-[#7d6330] dark:hover:text-[#bb9a5e] transition-colors">
            <ArrowLeft size={12} /> Communauté
          </Link>
        </div>

        <div className="grid lg:grid-cols-12 gap-4 h-[calc(100vh-9rem)] min-h-[34rem]">
          <aside className={`lg:col-span-4 ${autreUid ? 'hidden lg:flex' : 'flex'} flex-col rounded-[20px] border border-white/60 dark:border-white/10 bg-white/55 backdrop-blur-md dark:bg-[#2a2015]/55 overflow-hidden shadow-[0_10px_30px_-18px_rgba(58,49,38,0.3)]`}>
            <div className="px-4 py-3.5">
              <h1 className="font-serif text-sm text-[#2a2015] dark:text-white mb-2.5 flex items-center gap-2">
                <MessageCircle size={14} className="text-[#7d6330] dark:text-[#bb9a5e]" /> Vos conversations
              </h1>
              <div className="relative">
                <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#3a3126]/40 dark:text-white/40" />
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 rounded-full text-xs text-[#2a2015] dark:text-white placeholder:text-[#3a3126]/40 dark:placeholder:text-white/40 outline-none border border-[#3a3126]/10 dark:border-white/10 bg-white/60 dark:bg-white/5 focus:border-[#bb9a5e] transition-colors"
                />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              {filtres.length === 0 ? (
                <div className="p-8 text-center text-[#3a3126]/50 dark:text-white/45">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors border-b border-[#3a3126]/5 dark:border-white/5 ${actif ? 'bg-[#bb9a5e]/10' : 'hover:bg-[#bb9a5e]/5'}`}
                  >
                    <Avatar nom={nom} url={f.participantPhotos?.[u]} taille={40} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline gap-2">
                        <p className={`font-serif text-sm truncate ${actif ? 'text-[#7d6330] dark:text-[#bb9a5e]' : 'text-[#2a2015] dark:text-white'}`}>{nom}</p>
                        <span className="ml-auto text-[10px] tabular-nums text-[#3a3126]/40 dark:text-white/40 shrink-0">
                          {quand(f.lastMessageAt?.toMillis?.() ?? 0)}
                        </span>
                      </div>
                      <p className="text-[12px] text-[#3a3126]/60 dark:text-white/55 truncate mt-0.5">
                        {f.lastSenderUid === monUid ? 'Vous : ' : ''}{f.lastMessage || '…'}
                      </p>
                    </div>
                    {neuf && <span aria-label="Des messages non lus" className="w-2 h-2 rounded-full bg-[#bb9a5e] shrink-0" />}
                  </button>
                );
              })}
            </div>
          ...
          <section className={`lg:col-span-8 ${autreUid ? 'flex' : 'hidden lg:flex'} flex-col rounded-[20px] border border-white/60 dark:border-white/10 bg-white/55 backdrop-blur-md dark:bg-[#2a2015]/55 overflow-hidden shadow-[0_10px_30px_-18px_rgba(58,49,38,0.3)]`}>
            {!autreUid ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-[#3a3126]/50 dark:text-white/45 px-8">
              ...
              <>
                <header className="px-4 py-3 flex items-center gap-3 border-b border-[#3a3126]/10 dark:border-white/10">
                  <button onClick={() => navigate('/messages')} aria-label="Revenir à la liste" className="lg:hidden text-[#3a3126]/60 dark:text-white/60 hover:text-[#7d6330] dark:hover:text-[#bb9a5e] transition-colors">
                    <ChevronLeft size={18} />
                  </button>
                  <Link to={`/membre/${autreUid}`} className="flex items-center gap-3 hover:opacity-80 transition flex-1 min-w-0">
                    ...
                    <p className="font-serif text-sm text-[#2a2015] dark:text-white truncate">{nomAutre}</p>
                  </Link>
                </header>

                <div ref={zoneRef} className="flex-1 overflow-y-auto px-4 md:px-6 py-5 space-y-3" aria-live="polite">
                  {msgs.length === 0 ? (
                    <p className="text-center pt-16 text-sm text-[#3a3126]/50 dark:text-white/45">
                      ...
                      {msgs.map((m) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`max-w-[76%] px-4 py-2.5 text-sm whitespace-pre-wrap break-words rounded-[16px] ${mien ? 'bg-[#bb9a5e] text-[#2a2015]' : 'bg-white/70 dark:bg-white/10 text-[#2a2015] dark:text-white border border-[#3a3126]/10 dark:border-white/10'}`}>
                              {m.body}
                              <span className={`block text-[9px] tabular-nums mt-1.5 ${mien ? 'text-[#2a2015]/55' : 'text-[#3a3126]/45 dark:text-white/45'}`}>
                ...
                {avis && (
                  <p role="status" className="px-4 md:px-6 py-2 text-[11px] text-[#7d6330] dark:text-[#bb9a5e] border-t border-[#3a3126]/10 dark:border-white/10">
                    {avis}
                  </p>
                )}

                <form onSubmit={envoyer} className="px-4 md:px-6 py-4 flex items-end gap-3 border-t border-[#3a3126]/10 dark:border-white/10">
                  ...
                  <textarea
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl text-sm text-[#2a2015] dark:text-white placeholder:text-[#3a3126]/40 dark:placeholder:text-white/40 resize-none max-h-36 outline-none border border-[#3a3126]/10 dark:border-white/10 bg-white/60 dark:bg-white/5 focus:border-[#bb9a5e] transition-colors"
                  />
                  <div className="flex flex-col items-end gap-1.5">
                    {reste < 200 && <span aria-live="polite" className="text-[10px] tabular-nums text-[#3a3126]/50 dark:text-white/45">{reste}</span>}
                    <button
                      type="submit" disabled={envoi || !brouillon.trim()} aria-label="Envoyer"
                      className="inline-flex items-center justify-center w-11 h-11 bg-[#bb9a5e] text-[#2a2015] rounded-full hover:bg-[#a3823f] transition-colors disabled:opacity-35 disabled:cursor-not-allowed"
                    >
                      <Send size={15} />
                    </button>
                  </div>
