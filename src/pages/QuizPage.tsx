...
const QuizPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen dark:bg-[#16100a] text-[#2a2015] dark:text-white pt-36 pb-24">
      <div className="max-w-[1800px] mx-auto px-4 md:px-12">
        ...
        <div className="text-center mb-16">
          ...
          <div className="w-24 h-1 bg-[#bb9a5e] mt-6 mx-auto" />
        </div>

        {/* What is Ayurveda — intro block */}
        <div className="text-center mb-16 max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-serif mb-6 tracking-wide">{ay.whatIsTitle}</h2>
          <p className="text-[#2a2015]/80 dark:text-white/80 font-serif text-lg md:text-2xl leading-relaxed italic max-w-4xl mx-auto">{ay.whatIsText}</p>
          <div className="w-16 h-1 bg-[#bb9a5e] mx-auto mt-8" />
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center w-full gap-10 lg:gap-16 xl:gap-24 mb-16 px-4">
          <div className="relative flex items-center justify-center flex-shrink-0 lg:w-auto xl:w-1/2">
            ...
            {selectedDosha && (
              <div className="absolute z-30 inset-0 flex items-center justify-center">
                <div className="bg-white dark:bg-[#2a2015] p-8 rounded-[30px] shadow-2xl max-w-xs md:max-w-sm text-center relative border border-[#bb9a5e]/20">
                  <button onClick={() => setSelectedDosha(null)} className="absolute top-3 right-4 text-[#2a2015]/40 dark:text-white/40 hover:text-[#2a2015] text-2xl">×</button>
                  <h3 className="text-3xl font-serif font-bold mb-2 text-[#7d6330]">{selectedDosha.name}</h3>
                  <p className="text-xs font-bold uppercase tracking-widest text-[#2a2015]/50 dark:text-white/50 mb-4">{selectedDosha.elements}</p>
                  <div className="w-12 h-px bg-[#bb9a5e]/30 mx-auto mb-4" />
                  <p className="text-[#2a2015]/80 dark:text-white/80 text-sm leading-relaxed mb-4">{selectedDosha.definition}</p>
                  <p className="text-[#7d6330] font-serif italic">{selectedDosha.action}</p>
                  <div className="mt-6 pt-4 border-t border-[#2a2015]/10">
                    <button
                      onClick={() => { addDoshaOil(selectedDosha.name); setSelectedDosha(null); }}
                      className="w-full bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] py-2 rounded-full text-xs font-bold uppercase hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors shadow-md"
                    >
          ...
          <div className="lg:w-1/2 text-center lg:text-left max-w-xl">
            <span className="text-[#7d6330] uppercase tracking-[0.2em] text-xs font-semibold block mb-2">{ay.introTitle}</span>
            <h3 className="text-3xl md:text-5xl font-serif mb-6">{ay.title}</h3>
            <p className="text-[#2a2015]/70 dark:text-white/70 font-serif text-lg leading-relaxed mb-6 italic">{ay.introText}</p>
            <div className="bg-white dark:bg-[#2a2015]/60 border border-[#2a2015]/5 dark:border-white/5 p-8 rounded-[24px] shadow-lg mb-8">
              <p className="text-[#2a2015]/80 dark:text-white/80 leading-relaxed mb-4 font-medium">{ay.desc}</p>
              <p className="text-[#2a2015] dark:text-white font-bold">{ay.quizPrompt}</p>
            </div>
            <button
              onClick={openQuiz}
              className="inline-block bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] px-10 py-4 rounded-full font-bold uppercase tracking-widest text-sm shadow-lg hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors"
            >
              {ay.quizBtn}
            </button>
          </div>
          ...
          <div className="bg-white dark:bg-[#2a2015] w-full max-w-2xl rounded-[30px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-[#2a2015]/10 dark:border-white/10 flex justify-between items-center bg-[#f6f3ee] dark:bg-white/5">
              <div>
                ...
                <p className="text-xs text-[#7d6330] uppercase tracking-widest font-bold mt-1">
                  {quizState.result
              ...
              <button onClick={() => setQuizState({ ...quizState, isOpen: false, result: null })} className="text-[#2a2015]/40 hover:text-[#2a2015] dark:text-white/40 dark:hover:text-white">
                <i className="fa-solid fa-times text-xl" />
              </button>
            </div>
            {!quizState.result && (
              <div className="w-full h-1 bg-[#2a2015]/5">
                <div className="h-full bg-[#bb9a5e] transition-all duration-500" style={{ width: `${((quizState.step + (q ? 0 : 1)) / QUIZ_DATA.length) * 100}%` }} />
              </div>
            )}
            <div className="p-6 md:p-8 overflow-y-auto flex-1">
              {!quizState.result ? (
                q ? (
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-[#7d6330] mb-3">
                    ...
                    <div className="space-y-3">
                      {q.options.map((opt, idx) => {
                        ...
                        const stateClass = isFlash
                          ? 'border-[#bb9a5e] bg-[#bb9a5e]/10 shadow-[0_4px_18px_rgba(187, 154, 94,0.25)]'
                          : 'border-[#2a2015]/10 dark:border-white/10 bg-white/50 dark:bg-white/5 hover:border-[#bb9a5e] hover:bg-[#bb9a5e]/5';
                        return (
                          <button
                            key={idx}
                            type="button"
                          ...
                          >
                            <div className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center mt-0.5 transition-colors ${
                              isFlash ? 'border-[#bb9a5e] bg-[#bb9a5e]' : 'border-[#2a2015]/20 dark:border-white/20'
                            }`}>
                              {isFlash && <i className="fa-solid fa-check text-[#2a2015] text-[9px]" />}
                            </div>
                            <span className="block text-[#2a2015]/85 dark:text-white/85 text-sm leading-relaxed flex-1">
                    ...
                    <div className="mt-6 flex items-center justify-between text-[10px] uppercase tracking-widest font-bold">
                      <button
                        ...
                        className="text-[#2a2015]/50 dark:text-white/50 hover:text-[#7d6330] disabled:opacity-30 disabled:hover:text-inherit transition-colors"
                      >
                      ...
                      <button
                        type="button"
                        onClick={restart}
                        className="text-[#2a2015]/40 dark:text-white/40 hover:text-[#7d6330] transition-colors"
                      >
                  ...
                  <div className="text-center max-w-md mx-auto py-4">
                    ...
                    <h2 className="text-5xl font-serif font-bold text-[#7d6330] mb-4">{quizState.teaser?.dominant.name}</h2>
                    {quizState.teaser && (
                      <div className="flex justify-center gap-8 mb-6 text-sm text-[#2a2015]/70 dark:text-white/70">
                        {ALL_DOSHAS.map(d => (
                          <div key={d} className="flex flex-col items-center">
                            <span className="font-bold text-[#7d6330] text-lg">{quizState.teaser!.percentages[d]}%</span>
                    ...
                    {quizState.teaser?.dominant.elements && (
                      <p className="text-xs uppercase tracking-[0.2em] font-bold text-[#2a2015]/50 dark:text-white/50 mb-6">{quizState.teaser.dominant.elements}</p>
                    )}
                    <div className="border-t border-[#2a2015]/10 dark:border-white/10 pt-6 mt-6">
                      <div className="flex items-center gap-3 mb-4 justify-center">
                        <i className="fa-solid fa-lock text-[#7d6330] text-sm" />
                        <span className="text-xs uppercase tracking-[0.25em] font-bold text-[#2a2015]/70 dark:text-white/70">{lang === 'FR' ? 'Profil complet' : 'Full profile'}</span>
                      </div>
                      <p className="text-[#2a2015]/60 dark:text-white/60 mb-6 text-sm leading-relaxed">
                        {user
                      ...
                      {user ? (
                        <button onClick={handleQuizCompute} disabled={submitting}
                          className="w-full bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] py-4 rounded-full font-bold uppercase tracking-widest text-sm hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors shadow-lg flex items-center justify-center gap-2">
                          {submitting
                      ...
                      <p className="mt-4 text-[10px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40">
                ...
                <div className="text-center">
                  <p className="font-serif text-xl italic text-[#2a2015]/60 dark:text-white/60 mb-2">{lang === 'FR' ? `Votre nature dominante est :` : `Your dominant nature is:`}</p>
                  <h2 className="text-5xl font-serif font-bold mb-6 text-[#7d6330]">{quizState.result.dominant.name}</h2>
                  <div className="flex justify-center gap-8 mb-8 text-sm text-[#2a2015]/70 dark:text-white/70">
                    {ALL_DOSHAS.map(d => (
                      <div key={d} className="flex flex-col items-center">
                        <span className="font-bold text-[#7d6330]">{quizState.result.percentages[d]}%</span>
                        <span className="capitalize">{d}</span>
                      </div>
                    ))}
                  </div>
                  <div className="max-w-md mx-auto mb-8 bg-[#f6f3ee] dark:bg-white/5 p-6 rounded-xl border border-[#2a2015]/5">
                    <p className="text-[#2a2015]/80 dark:text-white/80 italic leading-relaxed">{quizState.result.dominant.definition}</p>
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <h3 className="font-serif text-xl md:text-2xl text-[#2a2015] dark:text-white mb-1">
                          {lang === 'FR' ? ritual.titleFR : ritual.titleEN}
                        </h3>
                        <p className="font-serif italic text-[#2a2015]/75 dark:text-white/75 text-sm md:text-base mb-3">
                          {lang === 'FR' ? ritual.subtitleFR : ritual.subtitleEN}
                        </p>
                        <p className="inline-block text-[10px] uppercase tracking-[0.25em] font-bold px-3 py-1 rounded-full bg-white/60 dark:bg-white/10 text-[#2a2015]/70 dark:text-white/70 mb-5">
                  ...
                  <button onClick={() => { addDoshaOil(quizState.result.dominant.name); setQuizState({ ...quizState, isOpen: false, result: null }); }}
                    className="w-full bg-[#bb9a5e] text-[#2a2015] py-3 rounded-full uppercase tracking-widest text-xs font-bold hover:bg-[#2a2015] hover:text-white transition-colors shadow-lg mb-4 max-w-sm mx-auto block">
                    {lang === 'FR' ? `Ajouter l'huile ${quizState.result.dominant.name}` : `Add ${quizState.result.dominant.name} oil`}
                  </button>
                  <button onClick={() => setQuizState({ ...quizState, isOpen: false, result: null })} className="text-[#2a2015]/50 dark:text-white/50 uppercase tracking-widest text-xs font-bold hover:text-[#7d6330] transition-colors">
