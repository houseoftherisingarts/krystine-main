    ...
    <div className="min-h-screen dark:bg-[#16100a] text-[#2a2015] dark:text-white pt-36 pb-24">
      <div className="max-w-3xl mx-auto px-6 md:px-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="mt-5 font-serif italic text-[#2a2015]/60 dark:text-white/60 max-w-xl mx-auto">
            <EditableText
          ...
          <div className="w-24 h-1 bg-[#bb9a5e] mx-auto mt-8" />
        </div>
        ...
        {result ? (
          ...
          <>
            {/* Progress */}
            <div className="flex items-center justify-between mb-6 text-[10px] uppercase tracking-widest font-bold text-[#2a2015]/50 dark:text-white/50">
            ...
            <div className="relative h-1 rounded-full bg-[#2a2015]/5 dark:bg-white/10 overflow-hidden mb-8">
              <div
                className="absolute inset-y-0 left-0 bg-[#bb9a5e] rounded-full transition-[width] duration-500 ease-out"
            ...
            {/* Question card */}
            {current && (
              <div className="bg-white dark:bg-[#2a2015] rounded-[24px] shadow-lg border border-[#2a2015]/5 dark:border-white/5 p-6 md:p-10">
                ...
                <div className="mt-6 pt-4 border-t border-[#2a2015]/10 dark:border-white/10 flex items-center justify-between text-[10px] uppercase tracking-widest font-bold text-[#2a2015]/50 dark:text-white/50">
                  <button
                    type="button"
                    onClick={goBack}
                    disabled={step === 0}
                    className="hover:text-[#7d6330] disabled:opacity-30 disabled:hover:text-inherit transition-colors"
                  >
                    <i className="fa-solid fa-arrow-left mr-1" /> {lang === 'FR' ? 'Précédent' : 'Back'}
                  </button>
                  <span className="text-[#2a2015]/30 dark:text-white/30">
...
const ResultCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <h2 className="text-3xl md:text-5xl font-serif text-[#2a2015] dark:text-white mb-6 leading-tight">
        {lang === 'FR' ? rec.titleFR : rec.titleEN}
      </h2>
      <p className="text-[#2a2015]/75 dark:text-white/75 font-serif italic text-lg leading-relaxed max-w-xl mx-auto mb-10">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        <button
          type="button"
          onClick={onPrimary}
          className="inline-flex items-center gap-2 bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] px-8 py-3.5 rounded-full font-bold uppercase tracking-widest text-sm hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors shadow-md"
        >
        ...
        {rec.secondaryHref && (
          <button
            type="button"
            onClick={goSecondary}
            className="inline-flex items-center gap-2 border border-[#2a2015]/15 dark:border-white/15 text-[#2a2015]/70 dark:text-white/70 px-6 py-3 rounded-full font-bold uppercase tracking-widest text-[11px] hover:border-[#bb9a5e] hover:text-[#7d6330] transition-colors"
          >
            {lang === 'FR' ? rec.secondaryCtaFR : rec.secondaryCtaEN}
          </button>
        )}
      </div>
      <div className="mt-10 pt-6 border-t border-[#2a2015]/10 dark:border-white/10">
        <button
          type="button"
          onClick={onRestart}
          className="text-[10px] uppercase tracking-widest font-bold text-[#2a2015]/40 dark:text-white/40 hover:text-[#7d6330] transition-colors"
        >
