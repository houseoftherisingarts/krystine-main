...
const Palier: React.FC<{ atteint: boolean; icone: string; titre: string; sous: string }> = ({ atteint, icone, titre, sous }) => (
  <li className={`flex items-start gap-3 rounded-2xl border px-3 py-2.5 transition-colors ${atteint ? 'border-[#bb9a5e]/70 bg-[#bb9a5e]/10' : 'border-white/50 bg-white/30 dark:border-white/10 dark:bg-white/5'}`}>
    <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${atteint ? 'bg-[#bb9a5e] text-[#2a2015]' : 'bg-[#3a3126]/8 text-[#7d6330] dark:bg-white/10'}`}>
      <i className={`fa-solid ${atteint ? 'fa-check' : icone} text-[11px]`} />
    </span>
    <span className="min-w-0">
      <span className={`block text-[12px] font-semibold leading-snug ${atteint ? 'text-[#3a3126] dark:text-white' : 'text-[#3a3126]/75 dark:text-white/70'}`}>{titre}</span>
      <span className="block text-[10.5px] text-[#3a3126]/55 dark:text-white/50">{sous}</span>
    </span>
  </li>
);
...
const ClientParrainage: React.FC<{ uid: string; lang: string }> = ({ uid, lang }) => {
  ...
  return (
    <div className="rounded-[24px] border border-white/60 bg-white/55 p-5 backdrop-blur-md dark:border-white/10 dark:bg-[#2a2015]/55">
      ...
      <p className="mt-2 text-xs leading-relaxed text-[#3a3126]/65 dark:text-white/60">
        {fr
      ...
      <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.22em] text-[#7d6330]">{fr ? 'Les badges' : 'Badges'}</p>
      ...
      <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.22em] text-[#7d6330]">{fr ? 'Les cadeaux' : 'Gifts'}</p>
      <p className="mt-1 text-[10.5px] text-[#3a3126]/55 dark:text-white/50">
      ...
      {filleules.length > 0 && (
        <>
          <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.22em] text-[#7d6330]">{fr ? 'Vos filleules' : 'Your invitees'}</p>
          <ul className="mt-2 space-y-1">
            {filleules.slice(0, 12).map(f => (
              <li key={f.uid} className="flex items-center gap-2 text-[11.5px] text-[#3a3126]/80 dark:text-white/75">
                <span className={`h-1.5 w-1.5 rounded-full ${f.achatCompte ? 'bg-[#bb9a5e]' : 'bg-[#3a3126]/25 dark:bg-white/25'}`} />
                <span className="truncate">{f.nom || (fr ? 'Une filleule' : 'An invitee')}</span>
                {f.achatCompte && <span className="ml-auto text-[9.5px] uppercase tracking-[0.15em] text-[#7d6330]">{fr ? 'en formation' : 'enrolled'}</span>}
              </li>
            ))}
            {filleules.length > 12 && (
              <li className="text-[10.5px] text-[#3a3126]/50">{fr ? `et ${filleules.length - 12} autres` : `and ${filleules.length - 12} more`}</li>
            )}
          </ul>
        </>
      )}
      ...
      {codeMarraine && (
        <p className="mt-4 text-[10.5px] text-[#3a3126]/50 dark:text-white/45">
