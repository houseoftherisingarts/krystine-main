    ...
    <div>
      ...
      {toast && (
        <div
          className={`mb-6 px-4 py-3 rounded-xl text-sm ${
            toast.kind === 'ok'
              ? 'bg-[#bb9a5e]/15 text-[#2a2015] dark:text-white border border-[#bb9a5e]/30'
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 items-center">
          ...
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-3">
              ...
              <button
                ...
                className="w-7 h-7 rounded-full flex items-center justify-center text-[#2a2015]/40 dark:text-white/40 hover:text-[#7d6330] hover:bg-white/40 dark:hover:bg-white/10 transition-colors"
              >
                <i className="fa-solid fa-rotate text-[11px]" />
              </button>
            </div>
            <div className="flex items-baseline justify-center md:justify-start gap-2 mb-3">
              <span className="text-6xl md:text-7xl font-serif text-[#2a2015] dark:text-white">{balance.balance}</span>
              <span className="text-lg text-[#2a2015]/50 dark:text-white/50 uppercase tracking-widest font-bold">
                {lang === 'FR' ? 'points' : 'points'}
              </span>
            </div>
            <p className="text-[11px] text-[#2a2015]/50 dark:text-white/50 uppercase tracking-widest mb-4">
          ...
          <div className="w-40 h-40 md:w-48 md:h-48 mx-auto md:mx-0 rounded-2xl overflow-hidden border border-[#2a2015]/5 dark:border-white/10 shadow-[0_12px_32px_rgba(58,37,30,0.08)] bg-white/50 dark:bg-white/5">
      ...
      {!welcomeClaimed ? (
        <button
          ...
          className="w-full mb-6 flex items-center justify-between gap-4 px-5 py-4 rounded-2xl border border-[#bb9a5e]/40 bg-gradient-to-r from-[#bb9a5e]/20 to-[#bb9a5e]/5 hover:from-[#bb9a5e]/30 hover:to-[#bb9a5e]/10 transition-colors text-left"
        >
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] font-bold text-[#7d6330] mb-1">
              {lang === 'FR' ? 'Cadeau de bienvenue' : 'Welcome gift'}
            </p>
            <p className="font-serif text-[#2a2015] dark:text-white">
              {lang === 'FR'
                ? `Réclamez vos ${POINTS.welcome} points de bienvenue.`
                : `Claim your ${POINTS.welcome} welcome points.`}
            </p>
          </div>
          <span className="shrink-0 inline-flex items-center gap-2 bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] px-5 py-2.5 rounded-full font-bold uppercase tracking-widest text-[11px]">
            {claimingWelcome
        ...
        <div className="mb-6 px-5 py-3 rounded-2xl border border-[#2a2015]/5 dark:border-white/5 bg-white/50 dark:bg-white/5 flex items-center gap-3 text-sm text-[#2a2015]/70 dark:text-white/70">
          <i className="fa-solid fa-check-circle text-[#7d6330]" />
            ...
            return (
              <div
                ...
                className={`rounded-2xl border p-5 transition-opacity ${
                  locked ? 'border-[#2a2015]/5 dark:border-white/5 bg-[#f6f3ee]/50 dark:bg-white/[0.03]'
                    : 'border-[#2a2015]/10 dark:border-white/10 bg-white dark:bg-white/5'
                }`}
              >
                <div className="flex items-baseline justify-between mb-2 gap-2 flex-wrap">
                  <span className="text-[10px] uppercase tracking-[0.25em] font-bold text-[#7d6330]">
                    {r.cost} pts
                    {r.oneShot && (
                      <span className="text-[#2a2015]/50 dark:text-white/50 font-normal tracking-normal normal-case ml-2">
                  ...
                  {!tierReached && tierLabel ? (
                    ...
                    <span className="text-[10px] uppercase tracking-[0.25em] text-[#2a2015]/40 dark:text-white/40">
                      {lang === 'FR' ? `Encore ${r.cost - balance.balance} pts` : `${r.cost - balance.balance} pts away`}
                    </span>
                  ) : null}
                </div>
                <h4 className="font-serif text-[#2a2015] dark:text-white text-lg mb-2">
                  {lang === 'FR' ? r.labelFR : r.labelEN}
                </h4>
                <p className="text-sm text-[#2a2015]/60 dark:text-white/60 mb-4 leading-relaxed">
                ...
                <button
                  ...
                  onClick={() => onRedeem(r)}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-bold uppercase tracking-widest text-[11px] bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {redeeming === r.id
      ...
      <section className="mb-10">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-4 rounded-2xl border border-[#2a2015]/10 dark:border-white/10 bg-white dark:bg-white/5 text-left hover:border-[#bb9a5e] transition-colors"
        >
          <span className="flex items-center gap-3">
            <i className="fa-solid fa-circle-info text-[#7d6330]" />
            <span className="font-serif text-[#2a2015] dark:text-white">
              {lang === 'FR' ? 'Comment gagner des points' : 'How to earn points'}
            </span>
          </span>
          <i className={`fa-solid fa-chevron-down text-[#2a2015]/40 dark:text-white/40 text-xs transition-transform ${guideOpen ? 'rotate-180' : ''}`} />
        </button>
        {guideOpen && (
          <ul className="mt-3 space-y-2 text-sm text-[#2a2015]/80 dark:text-white/80 bg-[#f6f3ee] dark:bg-white/5 rounded-2xl p-5">
            ...
            return (
              <li
                ...
                className={`rounded-xl border p-4 flex items-center gap-4 transition-colors ${
                  isCurrent
                    ? 'border-[#bb9a5e]/50 bg-[#bb9a5e]/5'
                    : reached
                      ? 'border-[#2a2015]/10 dark:border-white/10 bg-white dark:bg-white/5'
                      : 'border-[#2a2015]/5 dark:border-white/5 bg-[#f6f3ee]/50 dark:bg-white/[0.02] opacity-60'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-serif text-[#2a2015] dark:text-white">
                    ...
                    {isCurrent && <span className="ml-2 text-[10px] uppercase tracking-widest font-bold text-[#7d6330]">· {lang === 'FR' ? 'Actuel' : 'Current'}</span>}
                  </p>
                  <p className="text-[11px] uppercase tracking-widest text-[#2a2015]/50 dark:text-white/50">
                    {t.threshold} pts
                  </p>
                </div>
              </li>
      ...
      <section>
        ...
        {events.length === 0 ? (
          ...
          <ul className="space-y-2">
            {events.map(ev => {
              ...
              return (
                <li key={ev.id} className="flex items-center gap-4 border border-[#2a2015]/5 dark:border-white/5 rounded-xl p-3">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                    positive ? 'bg-[#bb9a5e]/15 text-[#7d6330]' : 'bg-red-50 text-red-500'
                  }`}>
                    <i className={`fa-solid ${l.icon}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[#2a2015] dark:text-white truncate">{lang === 'FR' ? l.fr : l.en}</p>
        ...
        {redemptions.length > 0 && (
          <div className="mt-6">
            <h4 className="text-[11px] uppercase tracking-widest font-bold text-[#2a2015]/60 dark:text-white/60 mb-3">
              {lang === 'FR' ? 'Mes récompenses' : 'My rewards'}
            </h4>
            <ul className="space-y-2">
              {redemptions.map(r => (
                <li key={r.id} className="flex items-center gap-3 border border-[#2a2015]/5 dark:border-white/5 rounded-xl p-3 text-sm">
                  <i className="fa-solid fa-gift text-[#7d6330]" />
                  <span className="flex-1 text-[#2a2015] dark:text-white">{r.rewardLabel}</span>
...
const GuideRow: React.FC<{ pts: number; label: string; note?: string }> = ({ pts, label, note }) => (
  <li className="flex items-center gap-3">
    <span className="w-10 text-right font-serif text-[#7d6330] font-bold">+{pts}</span>
    <span className="flex-1">{label}</span>
    {note && <span className="text-[9px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40 whitespace-nowrap">{note}</span>}
  </li>
);
