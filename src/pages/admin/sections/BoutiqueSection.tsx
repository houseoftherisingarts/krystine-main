    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h3 className="text-sm uppercase tracking-widest text-[#2a2015]/60 dark:text-white/60 font-bold mb-2">
              Redirect switch
            </h3>
            <p className="text-sm text-[#2a2015]/70 dark:text-white/70 max-w-xl">
          ...
          {settingsLoading ? (
            <i className="fa-solid fa-circle-notch fa-spin text-[#7d6330] text-xl" />
          ) : (
            <ToggleSwitch
        ...
        {savedAt && !settingsError && (
          <p className="mt-3 text-[11px] uppercase tracking-widest text-[#7d6330] font-bold">
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {COLLECTIONS.map(c => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-4 p-4 rounded-xl bg-[#f6f3ee] dark:bg-white/5 border border-[#2a2015]/5 dark:border-white/5">
                <div className="w-14 h-14 rounded-lg bg-cover bg-center shrink-0" style={{ backgroundImage: `url(${c.bannerImage})` }} />
                <div className="flex-1 min-w-0">
                  <p className="font-serif text-[#2a2015] dark:text-white truncate">{c.labelFR}</p>
                  <p className="text-[11px] text-[#2a2015]/60 dark:text-white/60 truncate">/boutique/{c.slug}</p>
                </div>
                <span className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full ${
                  n > 0 ? 'bg-[#bb9a5e]/10 text-[#7d6330]' : 'bg-[#2a2015]/5 dark:bg-white/5 text-[#2a2015]/40 dark:text-white/40'
                }`}>
      ...
      <Card className="p-6">
        ...
        {productsLoading && products.length === 0 ? (
          ...
          <div className="py-12 text-center text-[#2a2015]/50 dark:text-white/50 text-sm italic">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {assignments.map(({ product: p, collection }) => {
                  ...
                  return (
                  <tr key={p.id} className={`border-t border-[#2a2015]/5 dark:border-white/5 hover:bg-[#bb9a5e]/5 ${hidden ? 'opacity-60' : ''}`}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div
                          className="w-10 h-10 rounded-lg bg-cover bg-center shrink-0 bg-[#f6f3ee] dark:bg-white/5 border border-[#2a2015]/5 dark:border-white/10"
                          style={{ backgroundImage: p.featuredImage?.url ? `url(${p.featuredImage.url})` : undefined }}
                        />
                        <div className="min-w-0">
                          <p className="text-[#2a2015] dark:text-white truncate">{p.title}</p>
                          <p className="text-[11px] text-[#2a2015]/50 dark:text-white/50 truncate">{p.handle}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[#2a2015]/70 dark:text-white/70 hidden md:table-cell">{p.productType || '—'}</td>
                    <td className="px-4 py-3">
                      {collection ? (
                        <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest font-bold bg-[#bb9a5e]/10 text-[#7d6330] px-2.5 py-1 rounded-full">
                          {collection.labelFR}
                        </span>
                      ) : (
                      ...
                      {p.tags.length > 0 ? (
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {p.tags.slice(0, 4).map(t => (
                            <span key={t} className="text-[10px] uppercase tracking-widest text-[#2a2015]/50 dark:text-white/50 bg-[#2a2015]/5 dark:bg-white/5 px-2 py-0.5 rounded-full">{t}</span>
                          ))}
                          {p.tags.length > 4 && <span className="text-[10px] text-[#2a2015]/40 dark:text-white/40">+{p.tags.length - 4}</span>}
                        </div>
                      ) : (
                        <span className="text-[#2a2015]/30 dark:text-white/30">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-[#2a2015]/70 dark:text-white/70 hidden md:table-cell">
        ...
        <div className="mt-6 pt-5 border-t border-[#2a2015]/10 dark:border-white/10 flex items-center justify-between gap-4 flex-wrap">
          <p className="text-xs text-[#2a2015]/50 dark:text-white/50">
