...
const GroupsSection: React.FC = () => {
  ...
  if (loading) {
    return <div className="py-16 flex justify-center"><i className="fa-solid fa-circle-notch fa-spin text-[#7d6330] text-2xl" /></div>;
  }

  return (
    <div className="space-y-5">
      <Card className="p-5">
        <p className="text-sm text-[#2a2015]/70 dark:text-white/70 leading-relaxed">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="font-serif text-lg text-[#2a2015] dark:text-white">
          Groupes <span className="text-[#2a2015]/40 dark:text-white/40 text-sm">· {groups.length}</span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {groups.map(g => {
            ...
            return (
              <Card
                key={g.id}
                className={`p-5 transition-all hover:-translate-y-0.5 hover:shadow-md ${selectedId === g.id ? 'ring-2 ring-[#bb9a5e]/60' : ''}`}
              >
                <button type="button" onClick={() => setSelectedId(g.id!)} className="text-left w-full">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="font-serif text-[#2a2015] dark:text-white">{g.name}</h4>
                      {isSegment && (
                        <span className="text-[9px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full bg-[#bb9a5e]/15 text-[#7d6330]">
                          <i className="fa-solid fa-filter text-[9px] mr-1" />Segment
                        </span>
                      )}
                    </div>
                    <span className="text-[10px] uppercase tracking-widest font-bold text-[#2a2015]/50 dark:text-white/50 shrink-0">
                      {g.memberEmails?.length || 0} membre{(g.memberEmails?.length || 0) === 1 ? '' : 's'}
                    </span>
                  </div>
                  {g.description && (
                    <p className="text-xs text-[#2a2015]/60 dark:text-white/60 mt-2 leading-relaxed">{g.description}</p>
                  )}
                  <p className="text-[10px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40 mt-3">
                    {(g.memberUids?.length || 0)} avec compte · {Math.max(0, (g.memberEmails?.length || 0) - (g.memberUids?.length || 0))} courriel seul
                  </p>
                </button>
                {isSegment && (
                  <div className="mt-3 pt-3 border-t border-[#2a2015]/5 dark:border-white/10">
                    <GhostButton
...
const GroupDetail: React.FC<{
  ...
  return (
    <Card className="p-5 md:p-6 border-[#bb9a5e]/40">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div className="flex-1 min-w-0">
          {editing ? (
            ...
            <>
              <h3 className="font-serif text-xl text-[#2a2015] dark:text-white">{group.name}</h3>
              {group.description && (
                <p className="text-sm text-[#2a2015]/60 dark:text-white/60 mt-1 leading-relaxed">{group.description}</p>
              )}
            </>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={onClose} aria-label="Fermer" className="w-9 h-9 rounded-full flex items-center justify-center text-[#2a2015]/40 dark:text-white/40 hover:text-[#2a2015] dark:hover:text-white">
      ...
      {/* ── Members ── */}
      <div className="border-t border-[#2a2015]/5 dark:border-white/10 pt-5">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-[11px] uppercase tracking-widest font-bold text-[#2a2015]/60 dark:text-white/60">
        ...
        {memberRows.length === 0 ? (
          <p className="text-sm text-[#2a2015]/40 dark:text-white/40 italic py-4 text-center">Aucun membre dans ce groupe.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {memberRows.map(c => (
              <div key={c.email} className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl bg-[#f6f3ee] dark:bg-white/5 border border-[#2a2015]/5 dark:border-white/5">
                <div className="min-w-0">
                  <p className="text-sm text-[#2a2015] dark:text-white truncate">{c.name || c.email}</p>
                  <p className="text-[11px] text-[#2a2015]/50 dark:text-white/50 truncate">{c.displayEmail}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-[9px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full ${c.uid ? 'bg-[#bb9a5e]/15 text-[#7d6330]' : 'bg-[#2a2015]/5 dark:bg-white/10 text-[#2a2015]/50 dark:text-white/50'}`}>
                    {c.uid ? 'Compte' : 'Courriel'}
                  </span>
                  <button onClick={() => removeContact(c.email)} aria-label="Retirer" className="w-7 h-7 rounded-full text-[#2a2015]/30 dark:text-white/30 hover:text-red-500 transition-colors">
...
const ComposePanel: React.FC<{ group: MessagingGroup; memberCount: number; uidCount: number }> = ({ group, memberCount, uidCount }) => {
  ...
  return (
    <div className="border-t border-[#2a2015]/5 dark:border-white/10 mt-6 pt-5">
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-full text-[11px] uppercase tracking-widest font-bold border transition-colors ${
            mode === 'internal'
              ? 'bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] border-transparent'
              : 'border-[#2a2015]/10 dark:border-white/10 text-[#2a2015]/70 dark:text-white/70 hover:border-[#bb9a5e] hover:text-[#7d6330]'
          }`}
        >
...
const ContactPicker: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#2a2015]/60 backdrop-blur-md" onClick={onCancel}>
      <div className="relative w-full max-w-xl bg-white dark:bg-[#2a2015] rounded-[24px] shadow-2xl border border-[#bb9a5e]/20 p-6 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-serif text-lg text-[#2a2015] dark:text-white">Ajouter des membres</h3>
          <button onClick={onCancel} aria-label="Fermer" className="w-8 h-8 rounded-full text-[#2a2015]/40 dark:text-white/40 hover:text-[#2a2015] dark:hover:text-white">
            <i className="fa-solid fa-times" />
          </button>
        </div>
        ...
        <div className="mt-3 flex-1 overflow-y-auto space-y-1">
          {filtered.length === 0 ? (
            <p className="text-sm text-[#2a2015]/40 dark:text-white/40 italic py-6 text-center">Aucun contact.</p>
          ) : filtered.map(c => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg border transition-colors text-left ${
                  isSel
                    ? 'border-[#bb9a5e] bg-[#bb9a5e]/10'
                    : 'border-transparent hover:bg-[#f6f3ee] dark:hover:bg-white/5'
                }`}
              >
                <div className="min-w-0">
                  <p className="text-sm text-[#2a2015] dark:text-white truncate">{c.name || c.email}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-[9px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full ${c.isMember ? 'bg-[#bb9a5e]/15 text-[#7d6330]' : 'bg-[#2a2015]/5 dark:bg-white/10 text-[#2a2015]/50 dark:text-white/50'}`}>
                    {c.isMember ? 'Compte' : 'Courriel'}
                  </span>
                  <i className={`fa-solid ${isSel ? 'fa-check-circle text-[#7d6330]' : 'fa-circle text-[#2a2015]/20 dark:text-white/20'}`} />
                </div>
              </button>
            );
          })}
        ...
        <div className="mt-4 flex items-center justify-between">
          <p className="text-[11px] text-[#2a2015]/50 dark:text-white/50">{selected.size} sélectionné·e·s</p>
          <div className="flex gap-2">
