...
const STATUS_OPTIONS: { id: BookingStatus; label: string; color: string }[] = [
  { id: 'new',         label: 'Nouveau',     color: 'bg-[#bb9a5e]/10 text-[#7d6330]' },
...
const DetailRow: React.FC<{ label: string; value?: string | null }> = ({ label, value }) => {
  ...
  return (
    <div className="flex items-baseline gap-3 py-1 text-sm">
      ...
      <span className="text-[#2a2015]/85 dark:text-white/85 break-words">{value}</span>
    </div>
  );
};
            ...
            <div className="flex items-start gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  <h3 className="font-serif text-[#2a2015] dark:text-white">{b.name}</h3>
                  <span className={`text-[10px] uppercase tracking-widest font-bold px-2.5 py-0.5 rounded-full ${status.color}`}>{status.label}</span>
                  <span className="text-[10px] uppercase tracking-widest font-bold px-2.5 py-0.5 rounded-full bg-[#2a2015]/5 dark:bg-white/10 text-[#2a2015]/70 dark:text-white/70">
                    {sourceLabel}
                  </span>
                </div>
                <p className="text-xs text-[#2a2015]/50 dark:text-white/50 mt-1">
                  {b.email}
                  {b.phone ? ` · ${b.phone}` : ''}
                ...
                {isOpen && (
                  <div className="mt-5 border-t border-[#2a2015]/5 dark:border-white/10 pt-4 space-y-0.5">
                    ...
                    {b.message && (
                      <div className="pt-3">
                        ...
                        <p className="text-sm text-[#2a2015]/85 dark:text-white/85 leading-relaxed whitespace-pre-line">{b.message}</p>
                      </div>
                    )}
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <select
                  ...
                  className="text-xs px-3 py-2 rounded-lg border border-[#2a2015]/10 dark:border-white/10 bg-[#f6f3ee] dark:bg-white/5 text-[#2a2015] dark:text-white"
                >
