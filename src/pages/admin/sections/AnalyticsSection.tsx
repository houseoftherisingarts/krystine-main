  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div className="flex gap-1 p-1 rounded-full bg-white dark:bg-[#293027]/60 border border-[#293027]/5 dark:border-white/5">
          {RANGES.map(r => (
            <button
              key={r.id}
              onClick={() => setRange(r.id)}
              className={`px-4 py-1.5 text-[11px] uppercase tracking-widest font-bold rounded-full transition-colors ${range === r.id ? 'bg-[#293027] dark:bg-[#BA7B39] text-white dark:text-[#293027]' : 'text-[#293027]/60 dark:text-white/60 hover:text-[#8B4A2F]'}`}
            >{r.label}</button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {kpis.map(k => (
              <Card key={k.label} className="p-4">
                ...
                <p className="text-xl md:text-2xl font-serif text-[#293027] dark:text-white leading-none">{k.value}</p>
                <p className="text-[9px] uppercase tracking-widest text-[#293027]/50 dark:text-white/50 font-bold mt-2">{k.label}</p>
              </Card>
            ))}
          </div>
              ...
              {buckets.map(b => {
                ...
                return (
                  <div key={b.date} className="flex-1 flex flex-col justify-end group relative">
                    <div
                      className={`w-full ${isWeekend ? 'bg-[#BA7B39]/60' : 'bg-[#BA7B39]'} rounded-t-sm transition-all group-hover:opacity-100 opacity-80`}
                      style={{ height: `${Math.max(h, b.revenue > 0 ? 2 : 0)}%` }}
                    />
                    <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover:block bg-[#293027] text-white text-[10px] px-2 py-1 rounded whitespace-nowrap pointer-events-none z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="p-6">
              ...
              {topProducts.length === 0 ? (
                <p className="text-sm italic text-[#293027]/40 dark:text-white/40">Aucune vente dans cette période.</p>
              ) : (
                <ul className="space-y-3">
                  {topProducts.map((p, i) => (
                    <li key={p.title} className="flex items-center gap-3 text-sm">
                      <span className="text-[10px] uppercase tracking-widest font-bold text-[#8B4A2F] w-6">#{i + 1}</span>
                      <span className="flex-1 text-[#293027] dark:text-white truncate">{p.title}</span>
                      <span className="text-[#293027]/60 dark:text-white/60 text-xs">{p.qty}×</span>
                      <span className="text-[#8B4A2F] font-bold">{formatCurrency(p.revenue, currency)}</span>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <Card className="p-6">
              ...
              {filtered.slice(0, 8).length === 0 ? (
                <p className="text-sm italic text-[#293027]/40 dark:text-white/40">Aucune commande.</p>
              ) : (
                <ul className="space-y-2 text-sm">
                  {filtered.slice(0, 8).map(o => (
                    <li key={o.id} className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="font-mono text-xs text-[#293027]/60 dark:text-white/60">{o.name}</p>
                        <p className="text-[#293027] dark:text-white truncate">{o.customer?.name || o.email || '—'}</p>
                      </div>
                      ...
                      <span className="text-[#8B4A2F] font-bold">{formatCurrency(o.totalPrice, o.currency)}</span>
                    </li>
                  ))}
                </ul>
              )}
