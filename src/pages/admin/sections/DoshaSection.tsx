    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {(['vata', 'pitta', 'kapha'] as const).map(k => (
          <Card key={k} className="p-4 text-center">
            <p className="text-[10px] uppercase tracking-widest font-bold text-[#7d6330]">{k}</p>
            <p className="text-2xl font-serif text-[#2a2015] dark:text-white mt-1">{totals[k]}</p>
          </Card>
        ))}
      </div>

      <div className="flex items-center justify-between">
        <p className="text-sm text-[#2a2015]/60 dark:text-white/60">{rows.length} résultat{rows.length > 1 ? 's' : ''}</p>
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-t border-[#2a2015]/5 dark:border-white/5 hover:bg-[#bb9a5e]/5">
                <td className="px-4 py-3 text-[#2a2015] dark:text-white">{r.firstName} {r.lastName}</td>
                <td className="px-4 py-3 text-[#2a2015]/70 dark:text-white/70 hidden md:table-cell">{r.email}</td>
                <td className="px-4 py-3">
                  <span className="capitalize text-[#7d6330] font-bold">{r.dominant}</span>
                </td>
                <td className="px-4 py-3 text-[#2a2015]/50 dark:text-white/50 hidden md:table-cell font-mono text-xs">
                  V{r.vata} · P{r.pitta} · K{r.kapha}
                </td>
                <td className="px-4 py-3 text-[#2a2015]/50 dark:text-white/50 hidden md:table-cell">{r.createdAt?.toDate().toLocaleDateString('fr-CA') || '—'}</td>
