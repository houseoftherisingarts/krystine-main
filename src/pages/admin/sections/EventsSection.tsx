  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[#2a2015]/60 dark:text-white/60">{events.length} événement{events.length > 1 ? 's' : ''}</p>
        ...
        <div className="space-y-3">
          {events.map(e => (
            <Card key={e.id} className="p-5 flex items-center gap-5">
              <div className="w-20 h-20 rounded-xl bg-cover bg-center bg-[#f6f3ee] dark:bg-white/5 shrink-0" style={{ backgroundImage: e.imageUrl ? `url(${e.imageUrl})` : undefined }} />
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <h3 className="font-serif text-[#2a2015] dark:text-white truncate">{e.title}</h3>
                  {e.isFeatured && <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full bg-[#bb9a5e]/10 text-[#7d6330]">Vedette</span>}
                  {e.isPublished === false && <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full bg-[#2a2015]/10 dark:bg-white/10 text-[#2a2015]/60 dark:text-white/60">Brouillon</span>}
                </div>
                <p className="text-sm text-[#2a2015]/50 dark:text-white/50 truncate">{e.date}{e.location ? ` · ${e.location}` : ''}</p>
              </div>
