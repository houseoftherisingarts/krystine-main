...
const statusLabel: Record<string, { label: string; color: string }> = {
  draft:     { label: 'Brouillon',  color: 'bg-[#bb9a5e]/15 text-[#7d6330]' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <p className="text-sm text-[#2a2015]/60 dark:text-white/60">{items.length} infolettre{items.length > 1 ? 's' : ''}</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map(n => {
                ...
                return (
                  <tr key={n.id} className="border-t border-[#2a2015]/5 dark:border-white/5 hover:bg-[#bb9a5e]/5">
                    <td className="px-4 py-3 text-[#2a2015] dark:text-white font-serif">{n.title || '—'}</td>
                    <td className="px-4 py-3 text-[#2a2015]/70 dark:text-white/70 hidden md:table-cell truncate max-w-[280px]">{n.subject || '—'}</td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <span className={`text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full ${st.color}`}>{st.label}</span>
                    </td>
                    <td className="px-4 py-3 text-[#2a2015]/60 dark:text-white/60 hidden lg:table-cell">{n.stats?.recipients ?? '—'}</td>
                    <td className="px-4 py-3 text-[#2a2015]/50 dark:text-white/50 hidden md:table-cell">{n.updatedAt?.toDate().toLocaleDateString('fr-CA') || '—'}</td>
