...
const PreviewFrame: React.FC<{ blocks?: NewsletterBlock[]; subject?: string; preheader?: string; kind?: string; height?: number }> = ({ blocks, subject, preheader, kind, height = 900 }) => {
  ...
  return (
    <div className="relative rounded-[15px] overflow-hidden border border-[#2a2015]/10 dark:border-white/10 bg-[#EEE7DB]">
      {busy && <div className="absolute top-3 right-3 z-10 text-[10px] uppercase tracking-widest text-[#7d6330] bg-white/80 px-2 py-1 rounded-full"><i className="fa-solid fa-circle-notch fa-spin mr-1" />Rendu</div>}
