...
const DOSHA_TABS: { id: DoshaFilter; label: string; color: string }[] = [
  { id: 'all',       label: 'Tous',     color: 'border-[#293027]/10 dark:border-white/10' },
  ...
  { id: 'Tridoshic', label: 'Tridosha', color: 'border-[#BA7B39]/60 text-[#8B4A2F]' },
];
...
const SubmissionsSection: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {(cat === 'all' || cat === 'dosha') && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[10px] uppercase tracking-widest font-bold text-[#293027]/50 dark:text-white/50 mr-1">
            Dosha dominant
          </span>
          {DOSHA_TABS.map(tab => {
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          ...
          className="text-xs px-4 py-2.5 rounded-full border border-[#293027]/10 dark:border-white/10 bg-white dark:bg-white/5 text-[#293027] dark:text-white min-w-[220px]"
        >
          ...
          {TIME_TABS.map(tab => (
            <button
              ...
              className={`px-3 py-2 rounded-full text-[11px] uppercase tracking-widest font-bold border transition-colors ${
                time === tab.id
                  ? 'bg-[#BA7B39]/15 text-[#8B4A2F] border-[#BA7B39]/40'
                  : 'border-[#293027]/10 dark:border-white/10 text-[#293027]/60 dark:text-white/60 hover:text-[#8B4A2F]'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="relative flex-1 min-w-[200px]">
          <i className="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-[#293027]/30 dark:text-white/30 text-[11px]" />
          <input
            type="text"
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Rechercher nom, courriel, message…"
            className="w-full pl-10 pr-4 py-2.5 rounded-full border border-[#293027]/10 dark:border-white/10 bg-white dark:bg-white/5 text-sm text-[#293027] dark:text-white placeholder:text-[#293027]/40 dark:placeholder:text-white/40 focus:outline-none focus:border-[#BA7B39]"
          />
        </div>
              ...
              <Card key={s.id} className="px-5 py-4">
                <div className="w-full flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-baseline gap-2">
                      {s.memberUid ? (
                        <button
                          ...
                          className="font-serif text-[#293027] dark:text-white hover:text-[#8B4A2F] transition-colors inline-flex items-center gap-1.5"
                          title="Voir le profil client"
                        >
                          {s.name}
                          <i className="fa-solid fa-arrow-up-right-from-square text-[10px] text-[#8B4A2F]" />
                        </button>
                      ) : (
                        <h3 className="font-serif text-[#293027] dark:text-white">{s.name}</h3>
                      )}
                      ...
                      <span className="text-[10px] uppercase tracking-widest text-[#293027]/40 dark:text-white/40 ml-auto">
                        {dateStr}
                      </span>
                    </div>
                    <button
                    ...
                    >
                      <p className="text-xs text-[#293027]/60 dark:text-white/60 mt-1 truncate">
                        {s.email || '—'} · <span className="italic">{prettySource(s.source)}</span>
                      </p>
                      <p className="text-xs text-[#293027]/70 dark:text-white/70 mt-1 truncate">{s.summary}</p>
                    </button>
                  </div>
                  <button
                    type="button"
                    ...
                    className="shrink-0"
                  >
                    <i className={`fa-solid ${isOpen ? 'fa-chevron-up' : 'fa-chevron-down'} text-[#293027]/40 dark:text-white/40 text-xs mt-2`} />
                  </button>
                </div>

                {isOpen && (
                  <div className="mt-4 pl-[52px] border-t border-[#293027]/5 dark:border-white/10 pt-4 space-y-1">
                    {s.details.length === 0 ? (
                      <p className="text-xs text-[#293027]/40 dark:text-white/40 italic">Aucun détail structuré.</p>
                    ) : s.details.map((d, i) => (
                    ...
                    {s.message && (
                      <div className="pt-3">
                        <span className="block text-[10px] uppercase tracking-widest text-[#293027]/50 dark:text-white/50 font-bold mb-1">
                          Message
                        </span>
                        <p className="text-sm text-[#293027]/85 dark:text-white/85 leading-relaxed whitespace-pre-line">{s.message}</p>
                      </div>
                    )}
                    ...
                    {s.tags.length > 0 && (
                      <div className="pt-3 flex flex-wrap gap-1.5">
                        {s.tags.map(tag => (
                          <span key={tag} className="text-[10px] px-2 py-0.5 rounded-full bg-[#293027]/5 dark:bg-white/10 text-[#293027]/60 dark:text-white/60">
                            {tag}
                          </span>
                        ))}
                      </div>
