...
const COVER_STYLES = [
  'bg-gradient-to-br from-[#293027] to-[#4A3228]',
  'bg-gradient-to-br from-[#2D4A3E] to-[#293027]',
  'bg-gradient-to-br from-[#3D271F] to-[#5C372E]',
  'bg-gradient-to-br from-[#1A1A2E] to-[#293027]',
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[#293027]/60 dark:text-white/60">{posts.length} article{posts.length > 1 ? 's' : ''}</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {COVER_STYLES.map(c => (
                    <button
                      ...
                      className={`h-14 rounded-xl ${c} border-2 ${editing.coverStyle === c ? 'border-[#BA7B39]' : 'border-transparent'}`}
                    />
                  ))}
                </div>
              </div>
            ...
            <Card key={p.id} className="overflow-hidden flex flex-col">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                  {p.isPublished === false && <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full bg-[#293027]/10 dark:bg-white/10 text-[#293027]/60 dark:text-white/60">Brouillon</span>}
                  {p.date && <span className="text-[10px] uppercase tracking-widest text-[#293027]/40 dark:text-white/40">{p.date}</span>}
                </div>
                <h3 className="font-serif text-lg text-[#293027] dark:text-white mb-1">{p.title}</h3>
                {p.subtitle && <p className="text-sm italic text-[#293027]/60 dark:text-white/60 mb-2">{p.subtitle}</p>}
                <p className="text-sm text-[#293027]/70 dark:text-white/70 line-clamp-3 flex-1">{p.content}</p>
                <div className="flex gap-2 pt-4 mt-4 border-t border-[#293027]/5 dark:border-white/5">
