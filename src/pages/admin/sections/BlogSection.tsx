  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[#2a2015]/60 dark:text-white/60">{posts.length} article{posts.length > 1 ? 's' : ''}</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {COVER_STYLES.map(c => (
                    <button
                      ...
                      className={`h-14 rounded-xl ${c} border-2 ${editing.coverStyle === c ? 'border-[#bb9a5e]' : 'border-transparent'}`}
                    />
                  ))}
                </div>
              </div>
            ...
            <Card key={p.id} className="overflow-hidden flex flex-col">
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                  {p.isPublished === false && <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full bg-[#2a2015]/10 dark:bg-white/10 text-[#2a2015]/60 dark:text-white/60">Brouillon</span>}
                  {p.date && <span className="text-[10px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40">{p.date}</span>}
                </div>
                <h3 className="font-serif text-lg text-[#2a2015] dark:text-white mb-1">{p.title}</h3>
                {p.subtitle && <p className="text-sm italic text-[#2a2015]/60 dark:text-white/60 mb-2">{p.subtitle}</p>}
                <p className="text-sm text-[#2a2015]/70 dark:text-white/70 line-clamp-3 flex-1">{p.content}</p>
                <div className="flex gap-2 pt-4 mt-4 border-t border-[#2a2015]/5 dark:border-white/5">
