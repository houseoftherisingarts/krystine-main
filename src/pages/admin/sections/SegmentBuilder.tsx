  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-start justify-center p-4 bg-[#293027]/60 backdrop-blur-md overflow-y-auto" onClick={onClose}>
      <div className="relative w-full max-w-4xl my-8 bg-white dark:bg-[#293027] rounded-[28px] shadow-2xl border border-[#BA7B39]/20" onClick={e => e.stopPropagation()}>
        ...
        {/* Header */}
        <div className="px-6 md:px-8 py-5 border-b border-[#293027]/5 dark:border-white/10 flex items-start justify-between gap-3 sticky top-0 bg-white dark:bg-[#293027] rounded-t-[28px] z-10">
          <div>
            <span className="text-[#8B4A2F] uppercase tracking-[0.3em] text-[10px] font-bold block mb-1">
              Segment intelligent
            </span>
            <h2 className="font-serif text-2xl text-[#293027] dark:text-white">Nouveau groupe par segment</h2>
            <p className="text-sm text-[#293027]/60 dark:text-white/60 mt-1 font-serif italic">
              Composez des critères (dosha, points, achats, parcours…) — le comptage se met à jour en direct.
            </p>
          </div>
          <button onClick={onClose} aria-label="Fermer" className="w-9 h-9 rounded-full text-[#293027]/40 dark:text-white/40 hover:text-[#293027] dark:hover:text-white shrink-0">
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            <Card className="p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[11px] uppercase tracking-widest font-bold text-[#293027]/70 dark:text-white/70">
                  Critères
                </h3>
                <div className="flex items-center gap-1 p-1 rounded-full bg-[#EEE7DB] dark:bg-white/5">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                {pointsLoading && (
                  <span className="text-[11px] text-[#293027]/50 dark:text-white/50">
            ...
            <Card className="p-5">
              <div className="flex items-baseline justify-between mb-3">
                ...
                <p className="text-sm text-[#293027] dark:text-white">
                  <span className="font-serif text-2xl text-[#8B4A2F]">{matched.length}</span>{' '}
                  contact{matched.length === 1 ? '' : 's'} correspond{matched.length === 1 ? '' : 'ent'}
                </p>
              </div>

              {matched.length === 0 ? (
                <p className="text-sm text-[#293027]/40 dark:text-white/40 italic py-3">
                  Personne ne correspond aux critères actuels.
                </p>
              ) : (
                <div className="max-h-64 overflow-y-auto divide-y divide-[#293027]/5 dark:divide-white/5">
                  {matched.slice(0, 50).map(c => (
                    <div key={c.email} className="flex items-center justify-between py-2 text-sm">
                      <div className="min-w-0">
                        <p className="text-[#293027] dark:text-white truncate">{c.name}</p>
                      ...
                      <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest shrink-0">
                        {c.dosha && <span className="px-2 py-0.5 rounded-full bg-[#293027]/5 dark:bg-white/10 text-[#293027]/60 dark:text-white/60">{c.dosha}</span>}
                        {c.isMember && <span className="px-2 py-0.5 rounded-full bg-[#BA7B39]/15 text-[#8B4A2F] font-bold">Compte</span>}
                      </div>
                    </div>
                  ))}
                  {matched.length > 50 && (
                    <p className="py-2 text-[11px] text-[#293027]/40 dark:text-white/40 italic text-center">
...
const CriterionRow: React.FC<RowProps> = ({ criterion, onChange, onRemove, canRemove }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr_auto] gap-3 items-center p-3 rounded-xl bg-[#EEE7DB] dark:bg-white/5 border border-[#293027]/5 dark:border-white/5">

      {/* Field select */}
      <select
        value={criterion.field}
        onChange={e => setField(e.target.value as CriterionField)}
        className="px-3 py-2 rounded-lg border border-[#293027]/10 dark:border-white/10 bg-white dark:bg-[#293027]/80 text-sm text-[#293027] dark:text-white"
      >
        {FIELDS.map(f => (
      ...
      <div className="flex items-center gap-2">
        {field?.valueKind === 'enum' || field?.valueKind === 'boolean' ? (
          <select
            ...
            className="flex-1 px-3 py-2 rounded-lg border border-[#293027]/10 dark:border-white/10 bg-white dark:bg-[#293027]/80 text-sm text-[#293027] dark:text-white"
          >
        ...
        {field?.needsExtra && (
          <input
            ...
            className="w-40 px-3 py-2 rounded-lg border border-[#293027]/10 dark:border-white/10 bg-white dark:bg-[#293027]/80 text-sm text-[#293027] dark:text-white"
      ...
      {field?.description && (
        <p className="md:col-span-4 text-[11px] text-[#293027]/50 dark:text-white/50 italic mt-1">
          {field.description}
        </p>
      )}
    </div>
  ...
  <button
    ...
    className={`px-3 py-1.5 rounded-full text-[10px] uppercase tracking-widest font-bold transition-colors ${
      active
        ...
        : 'text-[#293027]/60 dark:text-white/60 hover:text-[#8B4A2F]'
    }`}
  >
    {children}
  </button>
