...
const AdminClientView: React.FC<Props> = ({ uid, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[85] bg-[#2a2015]/55 backdrop-blur-md overflow-y-auto" onClick={onClose}>
      <div
        className="min-h-full flex items-start justify-center p-4 md:p-8"
        onClick={e => e.stopPropagation()}
      >
        <div className="relative w-full max-w-5xl bg-[#f6f3ee] dark:bg-[#16100a] rounded-[24px] shadow-2xl border border-[#bb9a5e]/20 overflow-hidden">
          {/* Close */}
          <button
            onClick={onClose}
            aria-label="Fermer"
            className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-white/80 dark:bg-[#2a2015]/80 backdrop-blur flex items-center justify-center text-[#2a2015] dark:text-white hover:bg-[#bb9a5e] hover:text-white transition-colors"
          >
            <i className="fa-solid fa-times text-lg" />
          </button>

          {/* Read-only banner */}
          <div className="bg-[#bb9a5e]/10 border-b border-[#bb9a5e]/20 px-6 md:px-8 py-3 text-[10px] uppercase tracking-[0.3em] font-bold text-[#7d6330] flex items-center gap-2">
          ...
          <div className="p-6 md:p-8">
            <div className="bg-white dark:bg-[#2a2015] rounded-[24px] shadow-sm border border-[#2a2015]/5 dark:border-white/5 p-6 md:p-8 mb-6 flex flex-wrap items-center gap-5">
              <div className="relative">
                <div
                  className="w-20 h-20 rounded-full bg-cover bg-center bg-[#f6f3ee] dark:bg-white/5 border-2 border-[#bb9a5e]/30"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[#2a2015]/50 dark:text-white/50 mt-1">{member?.email || '—'}</p>
                {member?.joinedAt && (
                  <p className="text-[10px] uppercase tracking-[0.25em] text-[#2a2015]/40 dark:text-white/40 mt-1">
            ...
            <div className="bg-white dark:bg-[#2a2015] rounded-[24px] shadow-sm border border-[#2a2015]/5 dark:border-white/5 p-6 md:p-8">
              {member === undefined ? (
                <div className="py-12 flex justify-center">
                  <i className="fa-solid fa-circle-notch fa-spin text-[#7d6330] text-2xl" />
                </div>
              ) : member === null ? (
                <div className="py-12 text-center text-[#2a2015]/60 dark:text-white/60 font-serif italic">
...
const ProfileView: React.FC<{ member: MemberDoc }> = ({ member }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
    <div className="md:col-span-1">
      ...
      <div
        className="w-40 h-40 rounded-full overflow-hidden bg-[#f6f3ee] dark:bg-white/5 border-2 border-[#bb9a5e]/30 bg-cover bg-center"
        style={{ backgroundImage: member.photoURL ? `url(${member.photoURL})` : undefined }}
      >
        {!member.photoURL && (
          <div className="w-full h-full flex items-center justify-center text-[#2a2015]/30 dark:text-white/30">
...
const ReadField: React.FC<{ label: string; value?: string; accent?: boolean }> = ({ label, value, accent }) => (
  <div>
    <label className="block text-[10px] uppercase tracking-widest text-[#2a2015]/60 dark:text-white/60 font-bold mb-2">{label}</label>
    <p className={`px-4 py-3 rounded-xl border border-[#2a2015]/10 dark:border-white/10 bg-[#f6f3ee] dark:bg-white/5 text-sm ${
      accent ? 'text-[#7d6330] font-bold' : 'text-[#2a2015] dark:text-white'
    }`}>
      {value || '—'}
    </p>
  </div>
  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-[#2a2015]/60 dark:text-white/60 font-serif italic">Aucune commande pour l'instant.</p>
      </div>
    );
  }
  return (
    <div className="space-y-4">
      {orders.map(o => (
        <div key={o.id} className="border border-[#2a2015]/5 dark:border-white/5 rounded-[20px] p-5">
          ...
          {o.subtotal && <p className="text-sm font-bold text-[#7d6330]">Total : {o.subtotal}</p>}
          {o.trackingNumber && (
            <p className="mt-3 pt-3 border-t border-[#2a2015]/5 dark:border-white/5 text-sm">
              <span className="text-[10px] uppercase tracking-widest font-bold text-[#2a2015]/60 dark:text-white/60 block mb-1">Suivi</span>
              <span className="font-mono text-[#2a2015] dark:text-white">{o.trackingNumber}</span>
            </p>
          )}
        </div>
      ))}
...
const DoshaView: React.FC<{ member: MemberDoc; results: DoshaResult[]; products: ShopifyProduct[]; guideResponses: GuideResponse[] }> = ({ member, results, products, guideResponses }) => {
  ...
  if (!latest && !member.dosha) {
    return (
      <div className="text-center py-12 text-[#2a2015]/60 dark:text-white/60 font-serif italic">
  ...
  return (
    <div>
      ...
      >
        <p className="text-[10px] uppercase tracking-[0.3em] text-[#7d6330] font-bold mb-3">Dominance</p>
        <h2 className="text-5xl font-serif text-[#2a2015] dark:text-white mb-2">{dominant}</h2>
        {guidance && (
          <p className="font-serif italic text-[#2a2015]/70 dark:text-white/70">{guidance.fr}</p>
        )}
      </div>
      ...
      {latest && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {(['vata', 'pitta', 'kapha'] as const).map(d => {
            ...
            const color = DOSHA_GUIDANCE[label]?.color || '#bb9a5e';
      ...
      {[
        ...
        return (
          <div key={rec.kicker} className="rounded-[20px] border border-[#2a2015]/10 dark:border-white/10 bg-white dark:bg-white/5 p-5 md:p-6 mb-4 flex items-center gap-5">
            <div
              className="w-20 h-24 rounded-lg bg-cover bg-center shrink-0 bg-[#f6f3ee] dark:bg-[#2a2015] border border-[#2a2015]/5 dark:border-white/10"
              style={{ backgroundImage: product?.featuredImage?.url ? `url(${product.featuredImage.url})` : undefined }}
            />
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-[0.3em] text-[#7d6330] font-bold mb-1">
                {rec.kicker}
                {rec.pct !== null && <span className="text-[#2a2015]/50 dark:text-white/50 font-normal tracking-normal normal-case ml-2">· {rec.name} {rec.pct}%</span>}
              </p>
              {product ? (
                <>
                  <p className="font-serif text-lg text-[#2a2015] dark:text-white">{product.title}</p>
                  <p className="text-sm text-[#7d6330]">{variant ? formatMoney(variant.price, 'FR') : ''}</p>
                </>
              ) : (
                <p className="font-serif italic text-[#2a2015]/60 dark:text-white/60">
      ...
      {results.length > 0 && (
        <div className="mt-6">
          <h3 className="text-sm uppercase tracking-widest text-[#2a2015]/60 dark:text-white/60 font-bold mb-3">
            Historique des quiz
          </h3>
          <div className="space-y-2">
            {results.map(r => (
              <div key={r.id} className="border border-[#2a2015]/5 dark:border-white/5 rounded-xl p-3 text-sm">
                ...
                <span className="text-[10px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40 ml-3">
        ...
        <div className="mt-8">
          ...
          <div className="space-y-3">
            {guideResponses.map(g => (
              <div key={g.id} className="border border-[#2a2015]/5 dark:border-white/5 rounded-xl p-3 text-sm">
                <div className="flex items-baseline justify-between gap-3 mb-2">
                  <span className="text-[#7d6330] font-bold">{g.recommendationLabel || g.recommendationId}</span>
                  <span className="text-[10px] uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40">
                    {g.createdAt?.toDate().toLocaleDateString('fr-CA') || ''}
                  </span>
                </div>
                {g.answers?.length > 0 && (
                  <ul className="text-[11px] text-[#2a2015]/60 dark:text-white/60 space-y-1 pl-4 list-disc">
                    {g.answers.map((a, i) => (
                      <li key={i}>
    ...
    <div>
      ...
      >
        ...
        <div className="flex items-baseline justify-center gap-2 mb-3">
          <span className="text-5xl md:text-6xl font-serif text-[#2a2015] dark:text-white">{balance.balance}</span>
          <span className="text-sm text-[#2a2015]/50 dark:text-white/50 uppercase tracking-widest font-bold">pts</span>
        </div>
        <p className="text-[11px] text-[#2a2015]/50 dark:text-white/50 uppercase tracking-widest">
      ...
      {next && (
        <div className="mb-6">
          <div className="flex items-baseline justify-between mb-2 text-[11px] uppercase tracking-widest font-bold text-[#2a2015]/60 dark:text-white/60">
            <span>{current.labelFR}</span>
            <span>{pointsToNext} pts vers {next.labelFR}</span>
          </div>
          <div className="relative h-2 rounded-full bg-[#2a2015]/5 dark:bg-white/10 overflow-hidden">
      ...
      {driftDetected && (
        <div className="mb-4 px-4 py-3 rounded-xl border border-yellow-300/60 bg-yellow-50 dark:bg-yellow-900/20 flex items-center justify-between gap-3 text-sm">
          ...
          <button
            ...
            disabled={adjusting}
            className="shrink-0 inline-flex items-center gap-2 px-4 py-2 rounded-full text-[11px] uppercase tracking-widest font-bold bg-[#2a2015] dark:bg-[#bb9a5e] text-white dark:text-[#2a2015] hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors disabled:opacity-50"
          >
      ...
      <div className="mb-6 rounded-xl border border-[#2a2015]/10 dark:border-white/10 p-4 bg-[#f6f3ee]/60 dark:bg-white/5">
        <p className="text-[11px] uppercase tracking-widest font-bold text-[#2a2015]/70 dark:text-white/70 mb-3">
          <i className="fa-solid fa-scale-balanced mr-2 text-[#7d6330]" />
          Ajustement manuel
        </p>
        <div className="grid grid-cols-1 md:grid-cols-[120px_1fr_auto_auto] gap-3 items-center">
          <input
            ...
            placeholder="Montant"
            className="px-3 py-2 rounded-lg border border-[#2a2015]/10 dark:border-white/10 bg-white dark:bg-[#2a2015]/60 text-sm text-[#2a2015] dark:text-white outline-none focus:border-[#bb9a5e]"
          />
          <input
            type="text"
            value={adjustNote}
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[11px] uppercase tracking-widest font-bold bg-[#bb9a5e]/15 text-[#7d6330] hover:bg-[#bb9a5e] hover:text-[#2a2015] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
        ...
        {adjustFeedback && (
          <p className="mt-3 text-[11px] uppercase tracking-widest font-bold text-[#7d6330]">
            <i className="fa-solid fa-check mr-1" />
            {adjustFeedback}
          </p>
        )}
        <p className="mt-3 text-[11px] text-[#2a2015]/50 dark:text-white/50 leading-relaxed">
      ...
      {events.length === 0 ? (
        <p className="text-sm text-[#2a2015]/50 dark:text-white/50 italic font-serif py-4">
        ...
        <ul className="space-y-2">
          {events.map(ev => {
            ...
            return (
              <li key={ev.id} className="flex items-center gap-3 border border-[#2a2015]/5 dark:border-white/5 rounded-xl p-3 text-sm">
                <span className="text-[10px] uppercase tracking-widest font-bold bg-[#2a2015]/5 dark:bg-white/5 text-[#2a2015]/70 dark:text-white/70 px-2 py-0.5 rounded-full">
                  {ev.kind}
                </span>
                <span className="flex-1 text-[#2a2015]/70 dark:text-white/70 text-xs">
                  {ev.at?.toDate().toLocaleDateString('fr-CA') || ''}
                </span>
                <span className={`font-serif font-bold ${positive ? 'text-[#7d6330]' : 'text-red-500'}`}>
                  {positive ? '+' : ''}{ev.amount}
                </span>
              </li>
            );
