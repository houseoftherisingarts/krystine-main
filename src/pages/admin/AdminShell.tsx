...
const AdminShell: React.FC<Props> = ({ user, section, onSectionChange, children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0] dark:bg-[#050C1A] flex">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-[#0B1A36] text-white flex flex-col transform transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:z-0`}>
        <div className="px-6 pt-8 pb-6 border-b border-white/10">
          ...
          <p className="text-[10px] uppercase tracking-[0.3em] text-[#D4AF37] font-bold">Espace Auteure</p>
          ...
          {NAV.map(item => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-3 text-sm text-left transition-colors border-l-4 ${active ? 'bg-white/5 border-[#D4AF37] text-[#D4AF37]' : 'border-transparent text-white/70 hover:text-white hover:bg-white/5'}`}
              >
        ...
        <div className="px-6 py-4 border-t border-white/10 text-xs text-white/60">
          <div className="flex items-center gap-3 mb-3">
            {user.photoURL && <img src={user.photoURL} alt="" className="w-8 h-8 rounded-full border border-[#D4AF37]/30" />}
          ...
          <button onClick={logout} className="w-full text-left text-[11px] uppercase tracking-widest text-white/50 hover:text-red-400 transition-colors">
      ...
      <main className="flex-1 min-w-0">
        <header className="bg-white dark:bg-[#0B1A36] border-b border-[#0B1A36]/10 dark:border-white/10 px-6 py-4 flex items-center gap-4 sticky top-0 z-20">
          <button onClick={() => setMobileOpen(true)} className="lg:hidden w-8 h-8 text-[#0B1A36] dark:text-white">
            <i className="fa-solid fa-bars text-lg" />
          </button>
          <div className="flex-1">
            <h1 className="text-xl md:text-2xl font-serif text-[#0B1A36] dark:text-white">{current?.label}</h1>
          </div>
          <a href="/" target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest text-[#0B1A36]/50 dark:text-white/50 hover:text-[#D4AF37] transition-colors">
