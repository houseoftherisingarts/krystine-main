    ...
    <div className="min-h-screen dark:bg-[#16100a] pt-36 pb-24">
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        <div className="text-center mb-20">
          ...
          <h1 className="text-5xl md:text-7xl font-serif text-[#2a2015] dark:text-white">
          ...
          {t.items?.map((item: any, idx: number) => {
            ...
            return (
              ...
              >
                  ...
                  {item.cover ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#2a2015] text-white p-6">
                      {item.status === 'locked' ? (
                        ...
                        <i className="fa-solid fa-leaf text-2xl text-[#7d6330] mb-4 opacity-60" />
                      )}
                ...
                <h3 className="text-2xl font-serif text-[#2a2015] dark:text-white mb-2 group-hover:text-[#7d6330] transition-colors">{item.title}</h3>
                  ...
                  <div className="flex flex-col gap-3 w-full">
                    ...
                    {canOrder && variant ? (
                      ...
                      <button
                        disabled
                        title={shopError}
                        className="w-full bg-[#2a2015]/30 dark:bg-white/10 text-white/80 py-3 rounded-full font-bold uppercase tracking-widest text-xs cursor-not-allowed"
                      >
                      ...
                      <button
                        disabled
                        className="w-full bg-[#2a2015]/20 dark:bg-white/5 text-[#2a2015]/50 dark:text-white/50 py-3 rounded-full font-bold uppercase tracking-widest text-xs cursor-wait"
                      >
                        <i className="fa-solid fa-circle-notch fa-spin mr-2" />
        ...
        {(CONTENT[lang].media.details.book.items as unknown as any[]).find(i => i.detailedReviews)?.detailedReviews && (
          <div className="mt-24">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-serif text-[#2a2015] dark:text-white italic">
                {lang === 'FR' ? 'Ce qu\'elles en disent' : 'What they say'}
              </h2>
              <div className="w-16 h-1 bg-[#bb9a5e] mx-auto mt-4 opacity-50" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {(CONTENT[lang].media.details.book.items[0] as any).detailedReviews?.map((r: any, i: number) => (
                <div key={i} className="bg-white dark:bg-[#2a2015]/60 p-8 rounded-[24px] shadow-lg border border-[#2a2015]/5 dark:border-white/5 relative mt-4">
                  <div className="absolute -top-4 left-8 text-4xl text-[#7d6330] opacity-30 font-serif">"</div>
                  <div className="flex text-[#7d6330] mb-4 text-xs gap-1">{Array(r.rating).fill(0).map((_, j) => <i key={j} className="fa-solid fa-star" />)}</div>
                  <p className="text-[#2a2015]/80 dark:text-white/80 italic font-serif text-lg mb-6 leading-relaxed">"{r.text}"</p>
                  <p className="text-xs font-bold uppercase tracking-widest text-[#2a2015]/40 dark:text-white/40">{r.user}</p>
                </div>
              ))}
            </div>
          </div>
