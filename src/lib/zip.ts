...
//
// Le kit de presse se rend maintenant à l'écran (components/presse/VisuelPresse.tsx) plutôt que
// dans un script, si bien que le zip d'un seul geste devait suivre : il se fabrique au clic, à
// partir des visuels que la page vient de capturer. Des JPEG et des PNG sont déjà compressés,
// alors la méthode « stored » ne perd rien et tient en cinquante lignes, là où un vrai deflate
// demanderait une bibliothèque de plus.
//
// Vérification : node scripts/qa/zip.test.mjs (fabrique une archive et la fait valider par unzip -t).
