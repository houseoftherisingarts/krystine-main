...
async function run(path,name){
  ...
  const hasGlobalWordmark = await page.evaluate(()=>!!Array.from(document.querySelectorAll('a')).find(a=>/\/\?splash=1/.test(a.getAttribute('href')||'')));
  await page.screenshot({ path:`${OUT}/nav-${name}.png` });
  console.log(`${name}: <nav>=${navCount} globalNav=${hasGlobalWordmark}`);
  await page.close();
}
