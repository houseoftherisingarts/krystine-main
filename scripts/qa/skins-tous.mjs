...
const SKINS = ['medzo','nuit','coffee','aube','terre','foret','ocean','encre','lotus','feminite','nature','aurore','or-pur','vata','pitta','kapha'];
...
await fsdoc(`pointsEvents/adjust:${uid}:qa`, { uid: { stringValue: uid }, kind: { stringValue: 'adjust' }, amount: { integerValue: '9000' }, dedupKey: { stringValue: `adjust:${uid}:qa` }, at: { timestampValue: new Date().toISOString() } });
await fsdoc(`pointsEvents/redeem:${uid}:qa`, { uid: { stringValue: uid }, kind: { stringValue: 'redeem' }, amount: { integerValue: '-8188' }, dedupKey: { stringValue: `redeem:${uid}:qa` }, at: { timestampValue: new Date().toISOString() } });
await fsdoc(`memberPoints/${uid}`, { balance: { integerValue: '812' }, lifetime: { integerValue: '9000' }, dernierJour: { stringValue: jour }, serie: { integerValue: '3' } });
