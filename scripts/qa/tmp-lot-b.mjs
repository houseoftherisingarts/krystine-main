...
try {
  ...
  await aller(a, '/membres'); await shot(a, 'membres-toutes-1440');
  ...
  const accepter = a.locator('button', { hasText: /^\s*Accepter\s*$/ }).first();
  if (await accepter.count()) { await accepter.click(); await a.waitForTimeout(2500); console.log('accepté'); } else console.log('PAS de bouton Accepter');
  await aller(a, '/membres?vue=amies'); await shot(a, 'membres-amies-1440');
  await aller(a, `/membre/${B.uid}`); await shot(a, 'membre-autre-amies-1440');
  await aller(a, `/membre/${A.uid}`); await shot(a, 'membre-soi-1440');
  await aller(a, '/groupes'); await shot(a, 'groupes-1440');
  // le skin nuit
  await fsdoc(`members/${A.uid}`, { personnalisation: { mapValue: { fields: { banniere: S('iris'), skin: S('nuit') } } } }, ['personnalisation']);
  await aller(a, '/membres'); await shot(a, 'membres-nuit-1440');
  await aller(a, `/membre/${A.uid}`); await shot(a, 'membre-soi-nuit-1440');
  await fsdoc(`members/${A.uid}`, { personnalisation: { mapValue: { fields: { banniere: S('iris') } } } }, ['personnalisation']);
  ...
  await aller(m, `/membre/${A.uid}`); await shot(m, 'membre-soi-390');
  await aller(m, '/groupes'); await shot(m, 'groupes-390');
  ...
  // ── B, bureau : la fiche de A sans le Foyer ──
  const b = await ouvrir(B, 'Nathalie Landreville', false);
  await aller(b, `/membre/${A.uid}`); await shot(b, 'membre-garde-1440');
