...
Les bassins d'articles : `LEGENDAIRES = ['skin-vata', 'skin-pitta', 'skin-kapha']` (`coffres.ts:86`), `COMMUNS` (skins et bannières achetables, `:87`), rares de l'argent `:64` et de l'or `:75`. Les noms lisibles vivent dans `NOMS_COSMETIQUES` (`:102-109`), la valeur de rachat en niskas d'un article déjà possédé dans `VALEUR_COSMETIQUE` (`:95-101`, × 1,05 arrondi au supérieur, `:232`). Le catalogue client des skins avec palettes et rareté est `SKINS` dans `pointsConfig.ts:187-282`; `skinParCle` (`:283`).
